...
/*
 * intervalle : construit la liste [a, a+1, ..., b-1, b]
 */
...
/*
 * filtre_mult : enleve les multiples de n dans l
 */
liste_entier filtre_mult (int n,liste_entier l) {
  if (l == NULL) 
    ...
    if ((i_aux % n) != 0) {
      free(l); 
      return cons(i_aux,filtre_mult(n,l_aux));
    } else {
      free(l); 
      return filtre_mult(n,l_aux);
    }
  } 
}
